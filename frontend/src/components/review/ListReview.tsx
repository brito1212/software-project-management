                ...
                .map((review) => (
                    <div key={review.id} className={styles.review_container}>
                        <div className={styles.review_header}>
                            <img className={styles.user_image} src={getUserImage(review.user)} alt="Profile Image"/>
                            <span className={styles.username}>{review.user.username}</span>
                            <Rating initialValue={review.rate} size={28} readonly={true}/>
                            {review.user.id === user?.id && (
                                
                                <i className={`fa-solid fa-trash-can ${styles.trash_icon}`} onClick={() => handleClick({ reviewId: review.id })}></i>
                               
                            )}
                        </div>
                        <small className={styles.date}>{new Date(review.created_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</small>
                        <h3>{review.title}</h3>
                        <p>{review.content}</p>
                    </div>
                ))}
